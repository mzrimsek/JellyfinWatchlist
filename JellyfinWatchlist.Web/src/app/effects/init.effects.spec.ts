...
describe('InitEffects', () => {
  ...
  describe('loadWatchlist$', () => {
    it('should dispatch WatchlistActions.loadWatchlist when ROOT_EFFECTS_INIT is triggered', (done) => {
      const initAction = { type: ROOT_EFFECTS_INIT };
      ...
      effects.loadWatchlist$.subscribe((result) => {
        expect(result).toEqual(expectedAction);
        expect(result.type).toBe('[Watchlist] LoadWatchlist');
        done();
      });
    });
    ...
    it('should only respond to ROOT_EFFECTS_INIT action', (done) => {
      ...
      effects.loadWatchlist$.subscribe(() => {
        fail('Effect should not emit for non-ROOT_EFFECTS_INIT actions');
      });
    });
    ...
    it('should not modify the WatchlistActions.loadWatchlist action', (done) => {
      const initAction = { type: ROOT_EFFECTS_INIT };
      const expectedAction = WatchlistActions.loadWatchlist();
  ...
  describe('parallel initialization effects', () => {
    it('should trigger both getSystemInfo$ and loadWatchlist$ effects simultaneously', (done) => {
      const initAction = { type: ROOT_EFFECTS_INIT };
      let systemInfoEmitted = false;
      let watchlistEmitted = false;

      actions$ = of(initAction);

      effects.getSystemInfo$.subscribe((result) => {
        expect(result).toEqual(SystemInfoActions.get());
        systemInfoEmitted = true;
        checkCompletion();
      });

      effects.loadWatchlist$.subscribe((result) => {
        expect(result).toEqual(WatchlistActions.loadWatchlist());
        watchlistEmitted = true;
        checkCompletion();
      });

      function checkCompletion() {
        if (systemInfoEmitted && watchlistEmitted) {
          done();
        }
      }
    });

    it('should maintain independence between effects', (done) => {
      const initAction = { type: ROOT_EFFECTS_INIT };
      ...
      setTimeout(() => {
        expect(systemInfoResults.length).toBe(1);
        expect(watchlistResults.length).toBe(1);
        expect(systemInfoResults[0]).toEqual(SystemInfoActions.get());
        expect(watchlistResults[0]).toEqual(WatchlistActions.loadWatchlist());
        done();
      }, 100);
    });
  });
