...
describe('WatchlistService (Integration)', () => {
  ...
  describe('Database Operations', () => {
    ...
    it('should maintain data integrity with concurrent operations', async () => {
      ...
      const deletePromises = items.slice(0, 5).map(item => 
        service.delete(userId, item.id)
      );
  ...
  describe('Real Database Constraints and Validation', () => {    it('should enforce unique constraints properly', async () => {
      // Arrange
      const userId = INTEGRATION_TEST_CONSTANTS.USER_ID;
  ...
  describe('Error Handling with Real Database', () => {    it('should handle database connection issues gracefully', async () => {
      ...
      // Act & Assert
      await expect(service.getAllForUser(INTEGRATION_TEST_CONSTANTS.USER_ID))
